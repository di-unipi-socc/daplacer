...
hardReqs(S, N, HD) :-
	...
	getSecReqs(DataIds, SecReqs),
	subset(SecReqs, SecCaps),
...
servicePlacement([(_,S,Cs)|Ss], HW, NewHW, P, NewP) :-
	member((_, (N,Mode)),Cs), nodeOK(S,N,P,HW),
	servicePlacement(Ss, HW, NewHW, [on(S,(N,Mode))|P], NewP).
...
hwAllocation(P,AllocHW) :-
	findall((N,HW), (member(on(S,(N,_)), P), service(S,_,HW,_,_)), HWs),
