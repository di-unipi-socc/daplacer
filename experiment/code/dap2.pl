...
daplacer1(App, Placement, Routes) :-
  \+ deployment2(App, _, _, _), placement(App, Placement, Alloc, Routes),
  assert(deployment2(App, Placement, Alloc, Routes)).
daplacer2(App, NewPlacement, NewRoutes) :-
  deployment2(App, Placement, (AllocHW,AllocBW), Routes),
  newServices(Placement, NewServices),
  once(reasoningStep(Placement, (AllocHW,AllocBW), Routes, KOServices, TmpAllocBW, OkRoutes, OkPlacement)),
  %reasoningStep(Placement, (AllocHW,AllocBW), Routes, [], [], [], KOServices, TmpAllocBW, OkRoutes, OkPlacement),
  append(NewServices, KOServices, ServicesToPlace),
  partialPlacement(ServicesToPlace, (AllocHW,TmpAllocBW), OkRoutes, OkPlacement, NewAlloc, NewRoutes, NewPlacement),
  retract(deployment2(App, _, _, _)), assert(deployment2(App, NewPlacement, NewAlloc, NewRoutes)).
